...
@Component({
    selector: 'app-student-external-virtual-class-history',
    templateUrl: './student-external-virtual-class-history.component.html',
    styleUrls: ['./student-external-virtual-class-history.component.css'],
    encapsulation: ViewEncapsulation.None
})

export class StudentExternalVirtualClassHistoryComponent implements OnInit {
    ...
    constructor(
        ...
        this.studentId = this.route.snapshot.paramMap.get("username");
    }
    ...
    getDetails(){
        ...
        this._service.post('getMeetingsByStudentWithDate', params).subscribe(res => {
            this.allVirtualClassList = res.meetings;
    ...
    hideUnsuccessFull(){
        this.virtualClassList = [];
        this.allVirtualClassList.forEach(element => {
            if(element.student_participants.length){
                this.virtualClassList.push(element); 
            }
        });
        this.is_all_shown = false;
